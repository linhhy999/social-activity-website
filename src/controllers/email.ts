...
export let registered = async (req: Request, res: Response, next: NextFunction) => {
    ...
    // mail.registered(student.fullName, student.email, activyty.name, "http://" + req.get("host") + "/activity-detail/" + activyty._id);
    console.log("Sending " + ++i + " mail(s) to " + student.fullName + " " + student.email + " " + activyty.name + " " + "http://" + req.get("host") + "/activity-detail/" + activyty._id);
